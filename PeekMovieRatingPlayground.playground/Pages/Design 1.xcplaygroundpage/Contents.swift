...
extension Design {
    enum HIG {
        ...
        static let preferredContentSize = CGSize(width: 375, height: 668)
        static let tableCellHeight = preferredContentSize.width / 2 + padding
    }
}
...
final class MyCellViewComponents {
    ...
    private enum Constants {
        ...
        static let titleLabelZPos: CGFloat = 15
        
        static let topShadowHeight = height * 1
        ...
        static let topShadowLeftOrigin: CGPoint = .init(x: frontPosterWidth, y: .zero)
        static let topShadowRightOrigin: CGPoint = .zero
    ...
    lazy var titleLabel: UILabel = {
        ...
        label.layer.zPosition = Constants.titleLabelZPos
...
//    MARK: exposed func
    func setupViews(parent: UIView) {
        ...
        topShadow.frame = .init(origin: .zero, size: .init(width: Constants.blurredDeskWidth, height: Constants.topShadowHeight))
    ...
    func checkConstraints(for indexRow: Int) {
        if indexRow.isMultiple(of: 2) {
            ...
            topShadow.frame.origin = Constants.topShadowLeftOrigin
            
        } else {
            rightAlignedConstraints.forEach { $0.isActive = false }
            leftAlignedConstraints.forEach { $0.isActive = true }
            topShadow.frame.origin = Constants.topShadowRightOrigin
